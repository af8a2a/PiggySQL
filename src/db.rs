...
mod test {
    ...
    async fn test_crud_sql() -> Result<(), DatabaseError> {
        ...
        let _ =
            database.run("create table t2 (c int primary key, d int unsigned null, e datetime)").await?;
        ...
        let _ = database.run("create table t3 (a int primary key, b decimal(4,2))").await?;
        let _ = database.run("insert into t3 (a, b) values (1, 1111), (2, 2.01), (3, 3.00)").await?;
        let _ = database.run("insert into t3 (a, b) values (4, 4444), (5, 5222), (6, 1.00)").await?;
        ...
        println!("not like t1:");
        let tuples_not_like_t1 = database.run("select * from t1 where z not like '_b'").await?;
        ...
        println!("not in t1:");
        let tuples_not_in_t1 = database.run("select * from t1 where a not in (5, 29)").await?;
        ...
        println!("inner join:");
        let tuples_inner_join = database.run("select * from t1 inner join t2 on a = c").await?;
        println!("{}", create_table(&tuples_inner_join));

        println!("left join:");
        let tuples_left_join = database.run("select * from t1 left join t2 on a = c").await?;
        println!("{}", create_table(&tuples_left_join));

        println!("right join:");
        let tuples_right_join = database.run("select * from t1 right join t2 on a = c").await?;
        println!("{}", create_table(&tuples_right_join));

        println!("full join:");
        let tuples_full_join = database.run("select * from t1 full join t2 on a = c").await?;
        ...
        println!("group agg:");
        let tuples_group_agg = database.run("select c, max(d) from t2 group by c having c = 1").await?;
        ...
        let tuples_group_agg =
            database.run("select c, max(d) as max_d from t2 group by c having c = 1").await?;
        println!("{}", create_table(&tuples_group_agg));
        ...
        println!("time where:");
        let tuples_time_where_t2 = database.run("select (c + 1) from t2 where e > '2021-05-20'").await?;
        println!("{}", create_table(&tuples_time_where_t2));

        assert!(database.run("select max(d) from t2 group by c").await.is_err());
