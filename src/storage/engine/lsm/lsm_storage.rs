...
impl StorageEngine for LsmStorage {
    ...
    fn scan(&self, range:impl std::ops::RangeBounds<Vec<u8>>) -> Result<KvScan> {
        ...
        memtable_iters.push(Box::new(snapshot.memtable.scan((range.start_bound(),range.end_bound()))));
        for memtable in snapshot.imm_memtables.iter().rev() {
            memtable_iters.push(Box::new(memtable.scan((range.start_bound(),range.end_bound()))));
        }
        ...
        sstable_iters.reserve(snapshot.l0_sstables.len());
        for sstable in snapshot.l0_sstables.iter().rev() {
            sstable_iters.push(Box::new(SsTableIter::create(sstable.clone(), (range.start_bound(),range.end_bound()))?));
        }
