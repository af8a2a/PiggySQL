...
impl SsTable {
    ...
    pub fn read_block_cached(&self, block_idx: usize) -> Result<Arc<Block>> {
        if let Some(ref block_cache) = self.block_cache {
            let blk = block_cache
                ...
                .map_err(|e| DatabaseError::InternalError(e.to_string()))?;
            Ok(blk)
        } else {
    ...
    pub fn front_find_block_idx(&self, key: &[u8]) -> i32 {
        self.block_metas
            .partition_point(|meta| meta.first_key <= key) as i32
            - 1
    }
...
impl SsTableIter {
    ...
    pub fn create(table: Arc<SsTable>, range: impl RangeBounds<Vec<u8>>) -> Result<Self> {
        let mut this = Self::new(table)?;
        match range.start_bound() {
            Bound::Included(v) => {
                this.front_seek_to_key(&v, true)?;
            }
            Bound::Excluded(v) => {
                this.front_seek_to_key(&v, false)?;
            }
            Bound::Unbounded => {}
        };
        match range.end_bound() {
            Bound::Included(v) => {
                this.back_seek_to_key(&v, true)?;
            }
            Bound::Excluded(v) => {
                this.back_seek_to_key(&v, false)?;
            }
            Bound::Unbounded => {}
        }
        Ok(this)
    ...
    /// Create a new iterator and seek to the last key-value pair which < `key`.
    pub fn create_and_back_seek_to_key(
        table: Arc<SsTable>,
        key: &[u8],
        included: bool,
    ) -> Result<Self> {
        ...
        match block_idx >= 0 {
            true => {
                let mut block_iter = BlockIter::create_and_seek_to_key(
                    self.table.read_block_cached(block_idx as usize)?,
                    key,
                    included,
                );
...
impl StorageIter for SsTableIter {
    fn front_entry(&self) -> Option<(Vec<u8>, Vec<u8>)> {
        self.front_block_iter
            .as_ref()
            .map_or(None, |(_, iter)| iter.front_entry())
    }

    fn back_entry(&self) -> Option<(Vec<u8>, Vec<u8>)> {
        self.back_block_iter
            .as_ref()
            .map_or(None, |(_, iter)| iter.back_entry())
    }
        ...
        match (&self.front_block_iter, &self.back_block_iter) {
            ...
            (Some((front_idx, front_iter)), None) => {
                (0 <= *front_idx && *front_idx < self.table.num_of_blocks() as i32 - 1)
                    || (*front_idx == self.table.num_of_blocks() as i32 - 1
                        && front_iter.is_valid())
            }
