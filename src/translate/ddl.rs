use {
    super::{error::TranslateError, translate_object_name},
    crate::{
        ast::{
            ddl::{AlterTableOperation, Column, OperateFunctionArg}, expr::Expr, types::DataType
        },
        result::Result,
    },
    sqlparser::ast::{
...
pub fn translate_column_def(sql_column_def: &SqlColumnDef) -> Result<Column> {
    let SqlColumnDef {
        name,
        data_type,
        options,
...
pub fn translate_operate_function_arg(arg: &SqlOperateFunctionArg) -> Result<OperateFunctionArg> {
    ...
    let data_type = DataType::from(&arg.data_type)?;
    let default = arg.default_expr.as_ref().map(Expr::from).transpose()?;
