...
use crate::{
    ast::{
        ...
        expr::Expr,
        join::{Join, JoinConstraint, JoinExecutor, JoinOperator, TableWithJoins},
        query::{OrderByExpr, Query, Select, SelectItem, SetExpr, TableAlias, TableFactor, Values},
    },
    error::*,
};

use super::{
    aggregate_function::translate_function_arg_exprs, error::TranslateError, translate_idents,
...
impl Query {
    pub fn from(sql_query: &SqlQuery) -> Result<Self> {
        ...
        let order_by = order_by
            .iter()
            .map(OrderByExpr::from)
            .collect::<Result<_>>()?;
    
        let limit = limit.as_ref().map(Expr::from).transpose()?;
        let offset = offset
            .as_ref()
            .map(|offset| Expr::from(&offset.value))
            .transpose()?;
